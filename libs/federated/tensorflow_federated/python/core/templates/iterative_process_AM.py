...
class IterativeProcessAM:
  ...
  iterative_process = IterativeProcessAM(initialize_fn, next_fn)
  ...
  def __init__(self,
               ...
               next_adv_fn: computation_base.Computation,
               next_is_multi_arg: Optional[bool] = None):
    """Creates a `tff.templates.IterativeProcessAM`.
    ...
    #
    ##NEXT_ADV
    #py_typecheck.check_type(next_adv_fn, computation_base.Computation)
    #next_adv_parameter_type = next_adv_fn.type_signature.parameter
    #state_type = _infer_state_type(initialize_result_type, next_adv_parameter_type,
    #                               next_is_multi_arg)
    #
    #next_adv_result_type = next_adv_fn.type_signature.result
    #if state_type.is_assignable_from(next_adv_result_type):
    #  # The whole return value is the state type
    #  pass
    #elif (_is_nonempty_struct(next_adv_result_type) and
    #      state_type.is_assignable_from(next_adv_result_type[0])):
    #  # The first return value is state type
    #  pass
    #else:
    #  raise errors.TemplateStateNotAssignableError(
    #      f'The first return argument of `next_adv_fn` must be '
    #      f'assignable to its first input argument, but found\n'
    #      f'`next_adv_fn` which returns type:\n{next_adv_result_type}\n'
    #      f'which does not match its first input argument:\n{state_type}')

    self._state_type = state_type
    self._initialize_fn = initialize_fn
    self._next_fn = next_fn
    self._next_adv_fn = next_adv_fn
  ...
  def next(self) -> computation_base.Computation:
    ...
    by `tff.templates.IterativeProcessAM.initialize`), and the first (or only)
  ...
  @property
  def next_adv(self) -> computation_base.Computation:
    return self._next_adv_fn
...
def is_stateful(process: IterativeProcessAM) -> bool:
  ...
  Args:
    process: The `IterativeProcessAM` to test for statefulness.
