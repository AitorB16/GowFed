...
def gower_matrix_limit_cols(data_x, col_lim, data_y=None, weight=None, cat_features=None):  
    ...
    out = np.zeros((x_n_rows, col_lim), dtype=np.float32)
   ...
   # print(X_cat,X_num,Y_cat,Y_num)
    ...
    for i in range(x_n_rows):          
        ...
        res = gower_get(X_cat[i,:], 
                          ...
                          num_max) 
        #print(res.shape)
        out[i,j_start:col_lim]=res[j_start:col_lim]
        #if x_n_rows == y_n_rows: out[i:,j_start]=res
        
    return out

def sliced_gower_matrix_limit_cols(data_x, col_lim, skip_n_rows, data_y=None, weight=None, cat_features=None):  
    ...
    out = np.zeros((x_n_rows-skip_n_rows, col_lim), dtype=np.float32)
    ...
    for i in range(skip_n_rows, x_n_rows):          
        j_start = 0
        # call the main function
        res = gower_get(X_cat[i,:], 
                          X_num[i,:],
                          ...
                          num_ranges,
                          num_max) 
        out[i-skip_n_rows,j_start:col_lim]=res[j_start:col_lim]
    return out
